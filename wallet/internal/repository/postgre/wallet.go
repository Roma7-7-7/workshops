...
func (r *Repository) GetWalletTransactionsU(id string) (*models.Wallet, []*models.UserTransaction, error) {
	query, args, err := psql.Select("w.id", "w.balance", "w.user_id",
		"t.id", "t.credit_wallet_id", "t.debit_wallet_id", "t.amount", "t.type", "t.fee_amount", "t.fee_wallet_id",
		"cu.id", "du.id").
		From("wallets as w").
		LeftJoin("transactions as t on w.id = t.credit_wallet_id or w.id = t.debit_wallet_id").
		LeftJoin("users cu on cu.id = w.user_id").
		LeftJoin("users du on du.id = w.user_id").
		Where(sq.Eq{"w.id": id}).
		OrderBy("t.id").
		ToSql()
	...
	for rows.Next() {
		...
		var tType, tAmount, tFeeAmount sql.NullInt64

		if err = rows.Scan(&wallet.ID, &wallet.Balance, &wallet.UserID,
			&tID, &cwID, &dwID, &tAmount, &tType, &tFeeAmount, &fwID,
			&cuID, &duID); err != nil {
		...
		if tID.Valid {
			transactions = append(transactions, &models.UserTransaction{
				Transaction: models.Transaction{
					...
					FeeAmount:      models.AmountFromDB(tFeeAmount.Int64),
